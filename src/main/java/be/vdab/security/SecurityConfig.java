...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private static final String AUTHORITIES_BY_USERNAME = "select gebruikers.naam as username, rollen.naam as authorities from gebruikers inner join gebruikersrollen on gebruikers.id = gebruikersrollen.gebruikerid inner join rollen on rollen.id = gebruikersrollen.rolid"
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.formLogin().loginPage("/login").and().logout().logoutSuccessUrl("/").and().authorizeRequests()
				...
				.mvcMatchers("/werknemers").hasAnyAuthority(MAGAZIJNIER, HELPDESKMEDEWERKER).mvcMatchers(HttpMethod.PUT, "/filialen/*").hasAuthority(MANAGER)
